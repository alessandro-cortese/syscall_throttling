...
/* Simple limits to avoid complex allocations */
#define SCTH_MAX_PROG_LEN 16   /* TASK_COMM_LEN */
#define SCTH_MAX_LIST     256  /* maximum number of elements returned per list */

/* payload per ADD/DEL program */
struct scth_prog_req {
    char name[SCTH_MAX_PROG_LEN]; /* NUL-terminated or cut */
};
...
/* LIST response, same layout per prog/uid/syscall */
struct scth_list_resp {
    __u32 count;               /* in/out: user passes capacity, kernel writes actual count */
    __u32 reserved;
...
struct scth_stats {
    ...
    __u64 avg_blocked_threads_x1000;        /* average * 1000 */

    __u32 max_current_per_sec;  /* NEW: value applied in the current epoch */
    __u32 max_next_per_sec;     /* NEW: latest value set via ioctl; takes effect from the next epoch */
    __u32 monitor_on;
};
